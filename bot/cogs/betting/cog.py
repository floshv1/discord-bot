...
class BettingCog(commands.Cog):
    ...
    async def poll_fixtures_now(self) -> int:
        """Fetch upcoming fixtures from every provider and post a card for each new one.

        Returns the number of markets created. Safe to call any time — existing markets are skipped.
        """
        guild_id = self.bot.config.guild_id  # type: ignore[attr-defined]
        ...
        if not channel_id:
            return 0  # /setup betting hasn't been run yet
